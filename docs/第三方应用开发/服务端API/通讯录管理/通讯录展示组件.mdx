---
title: "通讯录展示组件"
generated_at: "2025-12-02 09:51:39"
update_time: 1568103081
source: "https://developer.work.weixin.qq.com/document/path/91958"
---
...
<span style="color: red">注意：进行agentConfig之前要确保用户已经通过企业微信登录应用的页面，</span>即用户已经通过**企业微信管理端跳转**（应用详情页中的“业务设置--前往服务商后台”）或[第三方登录授权](#90001/90143/91124)进入到应用的页面。
...
<span style="font-size: 13px; color: red">注意：为了保证在所有环境下表现正常，一旦 open-data 元素的内容发生了变化，必须调用 WWOpenData.bind 接口重新绑定一次</span>

**WWOpenData.on(event: string, callback: Function)**

<span style="font-size: 13px; color: grey">为兼容旧版本终端，使用前需要判断函数是否存在</span>
...
<span style="font-size: 13px; color: grey">启动 canvas 支持功能</span>

**WWOpenData.prefetch(params: PrefetchParams)**

<span style="font-size: 13px; color: grey">预加载通讯录数据</span>
...
```
<span style="font-size: 13px; color: red">注意：出于安全设计和浏览器策略，prefetch 接口必须在 https 的网页下调用，否则会报运行错误</span>
...
<span style="font-size: 13px; color: red">注意：当前小程序通讯录展示插件不支持显示头像</span>
...
<img src="http://p.qpic.cn/pic_wework/2473525963/20269a30d2c696c0a4193e0ad0174f0e6e738c8bbb4771c5/0" style="width:640px;"/>
<img src="http://p.qpic.cn/pic_wework/3422438713/c2c478d216fb43c23fd36d0ac1d0ff3ba62b16b2e38b7385/0" style="width:640px;"/>
